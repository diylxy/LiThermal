...
static bool g_legacy_mjpeg_dumping = false;
...
static void record_ui_stop_indicator()
{
    LOCKLV();
    if (tm_create_circle)
    {
    ...
    if (circle_REC && lv_obj_is_valid(circle_REC))
    {
        lv_obj_del(circle_REC);
        circle_REC = NULL;
    }
    if (lot_rec && lv_obj_is_valid(lot_rec))
    {
        lv_obj_del(lot_rec);
        lot_rec = NULL;
    }
...
static void record_ui_start_indicator()
{
    record_ui_stop_indicator();
    LOCKLV();
    ...
    tm_create_circle = lv_timer_create([](lv_timer_t *tm_out)
                                       {
                                           ...
                                           tm_circle = lv_timer_create([](lv_timer_t *tm)
                                                                       {
                                                                           if (!circle_REC || lv_obj_is_valid(circle_REC) == false)
                                                                           {
                                                                               lv_timer_del(tm);
                                                                               circle_REC = NULL;
                                                                               return;
...
void camera_record_toggle_dump_stream()
{
    if (cameraUtils.connected == false)
        return;
    if (recorder_is_recording() || g_legacy_mjpeg_dumping)
    {
        flash_bang_effect();
        record_ui_stop_indicator();

        if (g_legacy_mjpeg_dumping)
        {
            codec_enablePacketDumping(false, "");
            g_legacy_mjpeg_dumping = false;
        }
        else
        {
            recorder_stop();
        }
        cameraUtils.setAutoShutterCorrect(true);
        return;
    }
    record_ui_start_indicator();
    ...
    cameraUtils.setAutoShutterCorrect(false);
    codec_enablePacketDumping(false, "");
    {
        char jpeg_name[128];
        sprintf(jpeg_name, "%s.jpeg", name_partial);
        ...
        if (snapshot)
        {
            ok = tje_encode_to_file(jpeg_name, 320, 240, 4, snapshot->data);
            lv_snapshot_free(snapshot);
        }
        UNLOCKLV();
        if (!ok)
    ...
    remove(file_name_buffer);
    sprintf(file_name_buffer, "%s.mjpeg", name_partial);
    if (!recorder_start(file_name_buffer, 320, 240, 25))
    {
        codec_enablePacketDumping(true, file_name_buffer);
        g_legacy_mjpeg_dumping = true;
        return;
    }
}
